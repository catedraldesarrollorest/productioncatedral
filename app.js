...
function renderDashRecent() {
  ...
  tbody.innerHTML=recent.map(p=>`<tr>
    <td>${(p.produccion_insumos||[]).map(i=>i.mercancias?.nombre||'—').join(', ')||'—'}</td>
    <td>${(p.produccion_resultados||[]).map(r=>r.mercancias?.nombre||'—').join(', ')||'—'}</td>
    <td>${sumKgNeto([p]).toFixed(2)} kg</td>
    <td>${calcTotalMerma([p]).toFixed(2)} kg</td>
    <td class="${rendClass(calcRend([p]))}">${calcRend([p])}%</td>
    <td>${p.turno}</td><td>${badgeHTML(p.estado)}</td><td>${fmtDate(p.fecha)}</td>
  </tr>`).join('');
}
